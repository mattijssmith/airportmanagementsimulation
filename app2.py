...
class Airport:
    ...
    def add_capex_project(self, project_name, cost, capacity_increase, lead_time, loan_amount):
        if project_name == 'Cargo Hangar':
            ...
            self.equity -= equity_portion
            self.capex_cash_outflow += cost
            st.info(f"Project '{project_name}' initiated. It will be operational in {lead_time} years.")
            return True
    ...
    def update_for_new_year(self, gdp_growth, opex_change, aero_charge_change):
        self.year += 1
        st.write(f"--- Year {self.year} Simulation ---")
        ...
        if self.strategy == 'Cargo Airport':
            ...
            # Additional impact from completed projects
...
if 'airport' not in st.session_state:
    st.session_state.airport = Airport(initial_traffic=10_000_000, initial_equity=500_000_000, initial_assets=500_000_000, initial_opex_ratio=0.1, initial_asset_value=1_000_000_000, initial_cargo_tonnes=500_000)
...
if 'run_simulation_flag' not in st.session_state:
...
if st.session_state.airport.strategy is None:
    ...
    st.write(f"Current Number of Movements per peak hour: {st.session_state.airport.current_movements:.2f}")
    ...
    if st.button("Start Simulation"):
        ...
        st.success(f"You have chosen the **{st.session_state.airport.strategy}** strategy. Click 'Next Year' to continue.")
        st.session_state.run_simulation_flag = True
        st.rerun()

if st.session_state.run_simulation_flag:
    if st.session_state.airport.year >= 10:
        st.balloons()
        st.header("Simulation Complete!")
        st.write("You have reached the end of the 10-year simulation. Rerun the app to start over.")
    else:
        st.header(f"Decisions for Year {st.session_state.airport.year + 1}")
        gdp_growth = st.session_state.gdp_data.get(st.session_state.airport.year + 1, 2.0)
        ...
        if selected_project in ['New Terminal', 'Expand Runway', 'Non-Aero Retail Expansion']:
            cost = 150_000_000 if selected_project == 'New Terminal' else (250_000_000 if selected_project == 'Expand Runway' else 50_000_000)
        ...
        if st.button("Advance to Next Year"):
            if selected_project != 'None':
                ...
                cost = 150_000_000 if project_name == 'New Terminal' else (250_000_000 if project_name == 'Expand Runway' else (45_000_000 if project_name == 'Cargo Hangar' else 50_000_000))
                capacity_increase = 2_000_000 if project_name == 'New Terminal' else (3_000_000 if project_name == 'Expand Runway' else 200_000)
                lead_time = 3 if project_name in ['New Terminal', 'Expand Runway'] else 1
